...
{{/*
Helper to get any value with global fallback
*/}}
{{- define "application-sets.getValue" -}}
{{- $key := .key -}}
{{- $chartConfig := .chartConfig -}}
{{- $ctx := .ctx -}}

{{- if hasKey $chartConfig $key -}}
  {{- get $chartConfig $key -}}
{{- else if hasKey $ctx.Values.global $key -}}
  {{- get $ctx.Values.global $key -}}
{{- else -}}
  {{- .default -}}
{{- end -}}
{{- end -}}