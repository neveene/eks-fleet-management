{{/*
Main template for generating ApplicationSets
*/}}
{{- define "application-sets.application-set-generator" -}}
{{- $ctx := . -}}
{{- $values := $ctx.Values -}}
{{- $chartType := $values.chartType | default $values.global.chartType }}
{{- $namespace := $values.namespace | default $values.global.namespace }}
{{- $syncPolicy := $values.syncPolicy | default $values.global.syncPolicy -}}
{{- $syncPolicyAppSet := $values.syncPolicyAppSet | default $values.global.syncPolicyAppSet -}}
{{- $goTemplateOptions := $values.goTemplateOptions | default $values.global.goTemplateOptions -}}
{{- $repoURLGit := $values.repoURLGit | default $values.global.repoURLGit -}}
{{- $repoURLGitRevision := $values.repoURLGitRevision | default $values.global.repoURLGitRevision -}}
{{- $repoURLGitBasePath := $values.repoURLGitBasePath | default $values.global.repoURLGitBasePath -}}
{{- $valueFiles := $values.valueFiles | default $values.global.valueFiles -}}
{{- $valuesFilePrefix := $values.valuesFilePrefix | default $values.global.valuesFilePrefix -}}
{{- $useValuesFilePrefix := $values.useValuesFilePrefix | default $values.global.useValuesFilePrefix -}}
{{- $useSelectors := $values.useSelectors | default $values.global.useSelectors -}}
{{- $globalSelectors := $values.globalSelectors | default $values.global.globalSelectors -}}
...
metadata:
  ...
  annotations:
    {{- include "application-sets.annotations" $ctx | nindent 4 }}
    {{- if $chartConfig.annotationsAppSet }}{{- toYaml $chartConfig.annotationsAppSet | nindent 4 }}{{- end }}
  labels:
    {{- include "application-sets.labels" $ctx | nindent 4 }}
